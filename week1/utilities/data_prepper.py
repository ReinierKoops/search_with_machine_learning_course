...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        try:
            ...
            # Check if response is valid and contains hits
            if response and response['hits']['hits'] and len(response['hits']['hits']) > 0:
                ...
                # Get all the hits
                query_hits = response['hits']['hits']

                # Put the response in the proper feature format
                feature_results = self.__log_ltr_query_hits(feature_results, query_id, query_hits)

                # Convert to dataframe + set proper types
                frame = pd.DataFrame(feature_results)
                return frame.astype({'doc_id': 'int64', 'query_id': 'int64', 'sku': 'int64'})

            # It is not a valid response
            else:
                # For debug purposes save queries that weren't matched
                no_results[key] = query_doc_ids
        return None

    def __log_ltr_query_hits(self, feature_results, query_id, query_hits):
        for (idx, hit) in enumerate(query_hits):
            # Put in values found in hit
            feature_results["doc_id"].append(int(hit['_id']))
            feature_results["sku"].append(int(hit['_source']['sku'][0]))
            feature_results["query_id"].append(int(query_id)) # Makes joining easier later

            # Extract logged features from hit
            features = hit['fields']['_ltrlog'][0]['log_entry']
            feature_results = self.__log_ltr_query_hit_features(feature_results, features)

        return feature_results

    def __log_ltr_query_hit_features(self, feature_results, features):
        for idx, feature in enumerate(features):
            feat_name = feature.get('name')
            feat_val = feature.get('value', 0)

            # Check if its present in feature_results
